#' Apply the Bayesian two-stage normal-selection causal model with instrumental variables.
#'
#' @description The \code{ts.nnormal.s} function applies the Bayesian two-stage normal-selection causal model to the continuous treatment data.
...
#' @param n.chains Number of Markov chains. The default is 1.
#' @param n.iter Number of total iterations per chain (including burnin). The default is 50000.
...
#' @param n.thin Thinning rate. Must be a positive integer. The default is 1.
...
#' }
#' @export
ts.nnormal.s<-function(formula,data,m.ind, advanced=FALSE, adv.model,
                    b0=0, B0=1.0E-6, g0=0, G0 =1.0E-6, u0=0.001, U0=0.001, e0=0.001, E0=0.001,
                    l0=0,L0=1.0E-6, beta.start=NULL, gamma.start=NULL, u.start=NULL, e.start=NULL,
                    lambda0.start=1, lambda1.start=1,n.chains=1,n.burnin=5000,n.iter=10000,n.thin=1,
                    DIC,debug=FALSE,codaPkg=FALSE){
  ...
  inits<- function(){
    c(beta.start,gamma.start,u.start,e.start,lambda0=lambda0.start,lambda1=lambda1.start,y=list(y.int))
  }
