#' Apply the normal-based Bayesian two-stage causal model with instrumental variables.
#'
#' @description The \code{ts.nnormal} function applies the normal-based Bayesian two-stage causal model
...
#' Default of 0.001 is equivalent to the noninformative prior for the inverse Gamma distribution.
#' @param E0 The shape hyperparameter of the inverse Gamma distribution (prior for the the variance of the
#' normal distribution on the model residuals at the second stage).
#' Default of 0.001 is equivalent to the noninformative prior for the inverse Gamma distribution.
...
#' }
#' @export
ts.nnormal<-function(formula,data,advanced=FALSE, adv.model,
  ...
  if (advanced==FALSE){
    ...
    tmpModel <- paste0(L1,L2,L3,L4,L5,LFormulasBeta,LFormulasGamma,LN_1,LN,LPara,"}\n")
  }else{
  ...
  inits <- function(){
    c(beta.start,gamma.start,u.start,e.start)
  }
