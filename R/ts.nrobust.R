#' Apply the robust-based Bayesian two-stage causal model with instrumental variables.
#'
#' @description The \code{ts.nrobust} function applies the robust-based Bayesian two-stage causal model
...
#' @param n.iter Number of total iterations per chain (including burnin). The default is 10000.
...
#' }
#' @export
ts.nrobust<-function(formula,data,advanced=FALSE, adv.model,
                    b0=1,B0=1.0E-6, g0=0,G0=1.0E-6, u0=0.001,U0=0.001, e0=0.001,E0=0.001, v0=0,V0=100,
                    beta.start=NULL, gamma.start=NULL, u.start=NULL, e.start=NULL, df.start=5,
  ...
  if (advanced==FALSE){
    ...
    tmpModel <- paste0(L1,L2,L3,L4,L5,LFormulasBeta,LFormulasGamma,LN_1,LN,Ldf,LPara,"}\n")
  }else{
  ...
  inits<- function(){
    c(beta.start,gamma.start,u.start,e.start,df=df.start)
  }
