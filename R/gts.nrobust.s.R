#' Apply the generalized Bayesian two-stage robust-selection causal model with instrumental variables.
#'
#' @description The \code{gts.nrobust.s} function applies the generalized Bayesian two-stage
...
gts.nrobust.s<-function(formula,data,m.ind,advanced=FALSE, adv.model,
  ...
  if (length(levels(factor(x[,1])))!=2){
    ...
    if (advanced==FALSE){
      ...
      tmpModel <- paste0(L1,L2,L3,L4,L5,L6,L7,LFormulasBeta,LFormulasGamma,
                         LFormulasLambda,LN,Ldf,LPara,"}\n")
    }else{
    ...
    inits<- function(){
      c(beta.start,gamma.start,e.start,df=df.start,lambda0=l1,lambda1=l2,y=list(y.int))
    }
